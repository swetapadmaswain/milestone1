...
class LLMService:
    def rank_and_explain(
        ...
        while attempt <= LLM_MAX_RETRIES:
            try:
                ...
                validated = self._validate(ranked_rows, parsed)
                if validated:
                    return validated, False, None
                raise RuntimeError("Invalid LLM schema/output")
            except Exception:
                attempt += 1
    ...
    def _build_prompt(request: RecommendationRequest, ranked_rows: List[Dict]) -> str:
        return (
            ...
            f"User: location={request.location}, budget={request.budget}, cuisine={request.preferred_cuisine}, min_rating={request.min_rating}, extras={request.additional_preferences}\n"
            f"Candidates: {json.dumps(ranked_rows, ensure_ascii=True)}"
        )

    @staticmethod
    def _validate(ranked_rows: List[Dict], parsed: Dict) -> Dict[str, Dict]:
        items = parsed.get("recommendations", [])
        if not isinstance(items, list):
            return {}
        valid_names = {str(r["restaurant_name"]).strip().lower() for r in ranked_rows}
        out: Dict[str, Dict] = {}
        for item in items:
            ...
            if name not in valid_names or not isinstance(llm_score, (int, float)):
                continue
            out[name] = {
...
class PersonalizationService:
    ...
    def _cost_to_band_from_event(self, event: Dict) -> str:
        # Approximate budget affinity from interaction intent for events without cost payload.
        signal = event.get("signal_name")
        if signal in {"conversion", "like"}:
            return "medium"
        return "low"
...
class Recommender:
    ...
    def recommend(self, catalog: pd.DataFrame, request: RecommendationRequest) -> RecommendationRunResult:
        ...
        filtered = self._apply_filters(
            catalog, city, request.min_rating, min_cost, max_cost, cuisine_pref, apply_cuisine=True
        )
        if filtered.empty:
            relaxed_rating = max(request.min_rating - 0.5, 0.0)
            filtered = self._apply_filters(
                catalog, city, relaxed_rating, min_cost, max_cost, cuisine_pref, apply_cuisine=True
            )
        ...
        if filtered.empty:
            filtered = self._apply_filters(
                catalog, city, max(request.min_rating - 0.5, 0.0), min_cost, max_cost, cuisine_pref, apply_cuisine=False
            )
            if not filtered.empty:
        ...
        if filtered.empty:
            ...
            filtered = self._apply_filters(
                catalog,
                city,
                max(request.min_rating - 0.5, 0.0),
                widened_min,
                widened_max,
                cuisine_pref,
                apply_cuisine=False,
            ...
            if not filtered.empty:
                steps.append("Widened budget band while retaining location constraint.")
        if filtered.empty:
        ...
        llm_candidates = filtered.sort_values(
        ...
        llm_map, fallback_used, fallback_reason = self.llm.rank_and_explain(request, llm_payload)
        ...
        for _, row in top_n.iterrows():
            ...
            recs.append(
                RestaurantRecommendation(
                    ...
                    explanation=llm_map.get(
                        str(row["restaurant_name"]).strip().lower(), {}
                    ).get("explanation", self._fallback_explanation(row, request)),
                    confidence=llm_map.get(str(row["restaurant_name"]).strip().lower(), {}).get(
                        "confidence", "medium"
                    ),
                )
            )
    ...
    def _build_reason(
        ...
        return (
            f"Fits {request.preferred_cuisine} in {request.location}, "
            f"rating {row['rating']:.1f} clears minimum {request.min_rating:.1f}, "
            f"and blended score uses rule={score_breakdown['weight_rule']:.2f}, "
            f"llm={score_breakdown['weight_llm']:.2f}, "
            f"personalization={score_breakdown['weight_personal']:.2f}."
        )

    @staticmethod
    def _fallback_explanation(row: pd.Series, request: RecommendationRequest) -> str:
        return (
            f"Deterministic fallback: {row['restaurant_name']} matches cuisine/location preferences, "
            f"rating {row['rating']:.1f} satisfies threshold, and cost {row['estimated_cost']:.0f} "
            f"is aligned with {request.budget} budget."
        )
